-- Batch 1/3 (Set 2): parent + students + balances
-- Parent: 99990010 / colin.wong@hkedutech.com
-- Students: P1=39, P2=60, P3=33, P4=18
...
WHERE mobile_number = '99990010';

INSERT INTO public.parents (mobile_number, email)
SELECT '99990010', 'colin.wong@hkedutech.com'
WHERE NOT EXISTS (
  SELECT 1 FROM public.parents WHERE mobile_number = '99990010'
);
...
WHERE sa.session_id = qs.id
  AND (
    qs.session_token LIKE 'gearup_seed_99990010_%'
    OR qs.student_id IN (
      SELECT s.id
      FROM public.students s
      JOIN public.parents p ON p.id = s.parent_id
      WHERE p.mobile_number = '99990010'
        AND s.student_name ~ '^Test_[0-9]+$'
    )
  );

DELETE FROM public.quiz_sessions qs
WHERE qs.session_token LIKE 'gearup_seed_99990010_%'
   OR qs.student_id IN (
      SELECT s.id
      FROM public.students s
      JOIN public.parents p ON p.id = s.parent_id
