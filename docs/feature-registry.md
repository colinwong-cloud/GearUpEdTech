...
| F-REFERRAL-ADMIN-CONTACT | Admin 教師編號維護新增 tutor_mobile(必填) + tutor_email(可選) 並限制一個手機只能有一個啟用碼 | in_preview | release-critical | cursor/recover-missing-features-2d42 | #92 | not released | yes | Preview: `https://quiz-deploy-bix06wl29-colinwong-clouds-projects.vercel.app`; requires SQL `supabase_tutor_referral_contact_fields.sql` before production rollout |
