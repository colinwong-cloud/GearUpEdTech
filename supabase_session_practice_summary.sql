-- Session practice summary (Traditional Chinese) — 50–80 字, stored for CS/support + parent email
-- Run once in Supabase SQL Editor

ALTER TABLE public.quiz_sessions
  ADD COLUMN IF NOT EXISTS session_practice_summary text;

CREATE OR REPLACE FUNCTION public.save_session_practice_summary(
  p_session_id uuid,
  p_student_id uuid,
  p_summary text
)
...
BEGIN
  ...
  SET session_practice_summary = nullif(btrim(p_summary), '')
  WHERE id = p_session_id AND student_id = p_student_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.save_session_practice_summary(uuid, uuid, text) TO anon, authenticated, service_role;

-- Extend email RPC (merge with your existing get_quiz_email_data body if different)
