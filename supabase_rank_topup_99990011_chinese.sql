...
-- Target mobile: 99990011
-- Subject: Chinese
-- Goal: ensure each student under this parent reaches >= 100
--       lifetime Chinese questions_attempted.
-- ============================================================
...
BEGIN
  ...
  IF v_question_count < 10 THEN
    RAISE EXCEPTION 'Not enough Chinese questions (need >= 10, got %).', v_question_count;
  END IF;
  ...
  IF v_target_students = 0 THEN
    RAISE EXCEPTION 'No students found under mobile 99990011.';
  END IF;
END $$;
...
lifetime AS (
  ...
  WHERE qs.questions_attempted > 0
    AND lower(trim(qs.subject)) = 'chinese'
...
inserted_sessions AS (
  ...
  SELECT
    ...
    'rankfill-99990011-chinese-' || gen_random_uuid()::TEXT,
    NULL
...
JOIN LATERAL (
  ...
  FROM (
    ...
    WHERE lower(trim(q0.subject)) = 'chinese'
    ORDER BY
