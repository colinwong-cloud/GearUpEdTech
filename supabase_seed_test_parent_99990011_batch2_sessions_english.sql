-- Batch 2/3 (Set 1): English sessions + answers (200)
BEGIN;

DELETE FROM public.session_answers sa
USING public.quiz_sessions qs
WHERE sa.session_id = qs.id
  AND qs.session_token LIKE 'gearup_seed_99990011_english_%';

DELETE FROM public.quiz_sessions
WHERE session_token LIKE 'gearup_seed_99990011_english_%';
...
ins_sessions AS (
  ...
  SELECT
    ...
    'gearup_seed_99990011_english_' || gen_random_uuid()::text,
    NULL
  FROM session_rows sr
  RETURNING id, student_id, score, created_at
)
INSERT INTO public.session_answers (
  session_id, question_id, student_answer, is_correct, question_order, created_at
)
SELECT
  isess.id,
  qn.id,
...
SELECT COUNT(*)::int AS english_sessions
