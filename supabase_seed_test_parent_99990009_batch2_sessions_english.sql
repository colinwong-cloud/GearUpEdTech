-- ============================================================
-- Batch 2/3: Seed English sessions + answers
...
-- Remove previously seeded English sessions (safe re-run)
DELETE FROM public.session_answers sa
USING public.quiz_sessions qs
WHERE sa.session_id = qs.id
  AND qs.session_token LIKE 'gearup_seed_99990009_english_%';

DELETE FROM public.quiz_sessions
WHERE session_token LIKE 'gearup_seed_99990009_english_%';

-- Guard: ensure at least one eligible seeded student has >=10 English questions
DO $$
DECLARE
...
ins_sessions AS (
  ...
  SELECT
    ...
    'gearup_seed_99990009_english_' || gen_random_uuid()::text,
    NULL
