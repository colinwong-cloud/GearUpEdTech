-- Batch 1/3 (Set 1): parent + students + balances
-- Parent: 99990011 / colin.wong@hkedutech.com
-- Students: P1=41, P2=58, P3=30, P4=15
...
WHERE mobile_number = '99990011';

INSERT INTO public.parents (mobile_number, email)
SELECT '99990011', 'colin.wong@hkedutech.com'
WHERE NOT EXISTS (
  SELECT 1 FROM public.parents WHERE mobile_number = '99990011'
);
...
WHERE sa.session_id = qs.id
  AND (
    qs.session_token LIKE 'gearup_seed_99990011_%'
    OR qs.student_id IN (
      SELECT s.id
      FROM public.students s
      JOIN public.parents p ON p.id = s.parent_id
      WHERE p.mobile_number = '99990011'
        AND s.student_name ~ '^Test_[0-9]+$'
    )
  );

DELETE FROM public.quiz_sessions qs
WHERE qs.session_token LIKE 'gearup_seed_99990011_%'
   OR qs.student_id IN (
      SELECT s.id
      FROM public.students s
      JOIN public.parents p ON p.id = s.parent_id
...
SELECT s.grade_level, COUNT(*)::int AS students
FROM public.students s
JOIN public.parents p ON p.id = s.parent_id
WHERE p.mobile_number = '99990011'
  AND s.student_name ~ '^Test_[0-9]+$'
