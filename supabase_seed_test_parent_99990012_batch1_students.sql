-- Batch 1/3 (Set 3): parent + students + balances
-- Parent: 99990012 / colin.wong@hkedutech.com
...
WHERE mobile_number = '99990012';

INSERT INTO public.parents (mobile_number, email)
SELECT '99990012', 'colin.wong@hkedutech.com'
WHERE NOT EXISTS (
  SELECT 1 FROM public.parents WHERE mobile_number = '99990012'
);
...
WHERE sa.session_id = qs.id
  AND (
    qs.session_token LIKE 'gearup_seed_99990012_%'
    OR qs.student_id IN (
      SELECT s.id
...
WHERE qs.session_token LIKE 'gearup_seed_99990012_%'
   OR qs.student_id IN (
      SELECT s.id
      FROM public.students s
      JOIN public.parents p ON p.id = s.parent_id
