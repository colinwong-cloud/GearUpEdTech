-- Batch 3/3 (Set 1): Chinese sessions + answers (200)
BEGIN;

DELETE FROM public.session_answers sa
USING public.quiz_sessions qs
WHERE sa.session_id = qs.id
  AND qs.session_token LIKE 'gearup_seed_99990011_chinese_%';

DELETE FROM public.quiz_sessions
WHERE session_token LIKE 'gearup_seed_99990011_chinese_%';
...
ins_sessions AS (
  ...
  SELECT
    ...
    'gearup_seed_99990011_chinese_' || gen_random_uuid()::text,
    NULL
...
SELECT COUNT(*)::int AS chinese_sessions
