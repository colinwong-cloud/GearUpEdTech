-- ============================================================
-- Seed test data for parent mobile: 99990005
-- Email: colin.wong@hkedutech.com
--
-- What this script does:
-- 1) Creates/updates parent 99990005
...
--    - 500 sessions in Math
--    - 500 sessions in English
--    - 500 sessions in Chinese
--    - Each session has 10 answers
--    - Score randomized between 1 and 9 (10% to 90%)
--
-- Safe to rerun:
-- - Existing students under parent 99990005 are removed first
...
VALUES ('99990005', 'colin.wong@hkedutech.com', 'Test Parent 99990005')
ON CONFLICT (mobile_number) DO UPDATE
SET
  email = EXCLUDED.email,
  parent_name = EXCLUDED.parent_name;

CREATE TEMP TABLE tmp_seed_99990005_schools ON COMMIT PRESERVE ROWS AS
...
BEGIN
  SELECT count(*) INTO v_school_count FROM tmp_seed_99990005_schools;
...
student_plan AS (
  ...
  FROM parent_row p
  CROSS JOIN tmp_seed_99990005_schools sch
...
assigned_sessions AS (
  SELECT
    ...
    ('seed-99990005-' || lower(sp.subject) || '-' || gen_random_uuid()::TEXT) AS session_token
...
FROM tmp_seed_99990005_schools
ORDER BY district;
...
WHERE p.mobile_number = '99990005'
...
JOIN public.parents p ON p.id = s.parent_id
WHERE p.mobile_number = '99990005'
  AND lower(trim(qs.subject)) IN ('math', 'english', 'chinese')
