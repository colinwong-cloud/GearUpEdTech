-- Batch 2/3 (Set 2): English sessions + answers (200)
BEGIN;

DELETE FROM public.session_answers sa
USING public.quiz_sessions qs
WHERE sa.session_id = qs.id
  AND qs.session_token LIKE 'gearup_seed_99990010_english_%';

DELETE FROM public.quiz_sessions
WHERE session_token LIKE 'gearup_seed_99990010_english_%';
...
ins_sessions AS (
  ...
  SELECT
    ...
    'gearup_seed_99990010_english_' || gen_random_uuid()::text,
    NULL
