-- ============================================================
-- Seed test data for parent mobile: 99990003
-- Email: colin.wong@hkedutech.com
--
-- What this script does:
-- 1) Creates/updates parent 99990003
...
--    - 250 sessions in English
--    - 250 sessions in Chinese
--    - Each session has 10 answers
--    - Score randomized between 1 and 9 (10% to 90%)
--
-- Safe to rerun:
-- - Existing students under parent 99990003 are removed first
...
VALUES ('99990003', 'colin.wong@hkedutech.com', 'Test Parent 99990003')
ON CONFLICT (mobile_number) DO UPDATE
SET
  email = EXCLUDED.email,
  parent_name = EXCLUDED.parent_name;

CREATE TEMP TABLE tmp_seed_99990003_schools ON COMMIT DROP AS
...
BEGIN
  SELECT count(*) INTO v_school_count FROM tmp_seed_99990003_schools;
...
student_plan AS (
  ...
  FROM parent_row p
  CROSS JOIN tmp_seed_99990003_schools sch
...
session_plan AS (
  ...
  FROM (VALUES ('English'::TEXT, 250), ('Chinese'::TEXT, 250)) AS cfg(subject, total_count)
...
assigned_sessions AS (
  SELECT
    ...
    ('seed-99990003-' || lower(sp.subject) || '-' || gen_random_uuid()::TEXT) AS session_token
...
FROM tmp_seed_99990003_schools
ORDER BY district;
...
WHERE p.mobile_number = '99990003'
