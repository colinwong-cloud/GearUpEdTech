...
- **如仍超時**（兩道 RPC 各跑一輪仍觸及 PostgREST 單次請求上限）：每日批次已拆成兩個 Vercel Cron——`?part=rank`（0:00 UTC）與 `?part=grade`（0:02 UTC），見 `vercel.json`。（可選）在 Vercel 專案加入 `SUPABASE_SERVICE_ROLE_KEY` 讓 API 以 service role 呼叫 RPC。若 `?part=grade` 仍 timeout，請在 Supabase 執行 `supabase_optimize_grade_averages_batch.sql`（改寫圖表平均值彙總＋索引）。  
