...
| Date (approx) | Change |
|----------------|--------|
| 2026-04 | **註冊私隱同意**：註冊頁底部勾選「已閱讀並同意《私隱政策聲明》」方可按「**同意並繼續**」；連結開啟彈窗以 `fetch` 載入繁中 `.txt`若私隱檔案與題庫不在同一 Supabase 專案，請在 Vercel 設定 **`NEXT_PUBLIC_PRIVACY_STATEMENT_URL`** 為完整 `https://.../privacy_statment.txt` 連結。
