...
| Date (approx) | Change |
|----------------|--------|
| 2026-04 | `admin_today_business` / `admin_business_monthly` 需標為 **VOLATILE**（或移除函數內 `SET LOCAL statement_timeout`），否則 PostgreSQL 報錯：`SET is not allowed in a non-volatile function`。 |
