...
| Date (approx) | Change |
|----------------|--------|
| 2026-04 | **測試數據**：`supabase_seed_chinese_30_sessions_91917838.sql` — 為手機 **91917838** 下 **Loklok / Heihei** 各插入 30 節 **Chinese**、每節 10 題、正確率 40–90%；以 `session_practice_summary = '__SEED__:gearup_chinese_30__'` 標記以便重跑刪除（**不** `ALTER TABLE`）。需已存在欄位 `session_practice_summary`（見 `supabase_session_practice_summary.sql`）。計劃：`test_plan_seed_chinese_30_sessions_91917838.md`。 |
