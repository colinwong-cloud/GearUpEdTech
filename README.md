...
- **如仍超時**（兩道 RPC 各跑一輪仍觸及 PostgREST 單次請求上限）：每日批次已拆成兩個 Vercel Cron——`?part=rank`（0:00 UTC）與 `?part=grade`（0:02 UTC），見 `vercel.json`。（可選）在 Vercel 專案加入 `SUPABASE_SERVICE_ROLE_KEY` 讓 API 以 service role 呼叫 RPC。先執行 `supabase_optimize_grade_averages_batch.sql`；若 `?part=grade` 仍超時，再執行 `supabase_split_grade_averages_cron.sql`（依年級分多次 `RPC`，應用程式已支援）。  
