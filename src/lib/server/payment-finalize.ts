  ...
  if (!snapshotHasRecurringLinkage(snapshot)) {
    console.error(
      ...
      JSON.stringify({
        ...
        has_customer_id: Boolean(snapshot.customerId),
        has_payment_consent_id: Boolean(snapshot.paymentConsentId),
        has_payment_method_id: Boolean(snapshot.paymentMethodId),
        has_payment_method_type: Boolean(snapshot.paymentMethodType),
      })
    );
    throw new Error(
