...
interface PaymentRecurringMonitorUserRow {
  ...
  this_month_payment_status: "success" | "pending" | "failed" | "no_attempt";
  this_month_last_attempt_at: string | null;
  this_month_latest_paid_at: string | null;
  this_month_paid_amount_hkd: number;
  recurring_last_error: string | null;
  recurring_last_order_status: string | null;
  recurring_last_charged_at: string | null;
}
...
interface PaymentRecurringMonitorResult {
  ...
  totals: {
    ...
    recurring_failed: number;
    recurring_cancelled: number;
    recurring_no_profile: number;
    this_month_success: number;
    this_month_pending: number;
    this_month_failed: number;
    this_month_no_attempt: number;
  };
...
function formatRecurringStatusLabel(status: string | null | undefined): string {
  ...
  if (key === "active") return "續費中";
  if (key === "paused") return "已暫停";
  if (key === "failed") return "續費失敗";
  if (key === "cancelled") return "已取消";
  if (key === "no_profile") return "未設定自動續費";
  return key || "未知";
}

function formatMonthPaymentStatusLabel(status: string): string {
        ...
        {recurringMonitor && (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm">
              ...
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">本月續費成功</p>
                <p className="font-semibold text-emerald-700">{recurringMonitor.totals.this_month_success}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">本月續費待處理</p>
                <p className="font-semibold text-amber-700">{recurringMonitor.totals.this_month_pending}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">本月續費失敗</p>
                <p className="font-semibold text-red-700">{recurringMonitor.totals.this_month_failed}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">本月未嘗試</p>
                <p className="font-semibold text-gray-700">{recurringMonitor.totals.this_month_no_attempt}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm">
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">續費中</p>
                <p className="font-semibold text-indigo-700">{recurringMonitor.totals.recurring_active}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">已暫停</p>
                <p className="font-semibold text-gray-700">{recurringMonitor.totals.recurring_paused}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">續費失敗</p>
                <p className="font-semibold text-red-700">{recurringMonitor.totals.recurring_failed}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">已取消</p>
                <p className="font-semibold text-gray-700">{recurringMonitor.totals.recurring_cancelled}</p>
              </div>
              <div className="rounded-lg border border-gray-100 p-3">
                <p className="text-xs text-gray-500 mb-1">未設定自動續費</p>
                <p className="font-semibold text-gray-700">{recurringMonitor.totals.recurring_no_profile}</p>
              </div>
            </div>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="text-left text-gray-500 border-b">
                    ...
                    <th className="py-2 pr-3">本月已入帳金額 (HKD)</th>
                ...
                <tbody>
                  {recurringMonitor.users.map((row) => (
                    <tr key={row.parent_id} className="border-b border-gray-100">
                      ...
                      <td className="py-2 pr-3 font-mono">{formatHkdAmount(row.this_month_paid_amount_hkd)}</td>
