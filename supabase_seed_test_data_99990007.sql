-- ============================================================
-- Seed test data for parent mobile: 99990007
-- Email: colin.wong@hkedutech.com
--
-- What this script does:
-- 1) Creates/updates parent 99990007
...
--
-- Safe to rerun:
-- - Existing students under parent 99990007 are removed first
...
VALUES ('99990007', 'colin.wong@hkedutech.com', 'Test Parent 99990007')
ON CONFLICT (mobile_number) DO UPDATE
SET
  email = EXCLUDED.email,
  parent_name = EXCLUDED.parent_name;

CREATE TEMP TABLE tmp_seed_99990007_schools ON COMMIT PRESERVE ROWS AS
...
BEGIN
  SELECT count(*) INTO v_school_count FROM tmp_seed_99990007_schools;
...
student_plan AS (
  ...
  FROM parent_row p
  CROSS JOIN tmp_seed_99990007_schools sch
...
assigned_sessions AS (
  SELECT
    ...
    ('seed-99990007-' || lower(sp.subject) || '-' || gen_random_uuid()::TEXT) AS session_token
...
FROM tmp_seed_99990007_schools
ORDER BY district;
...
WHERE p.mobile_number = '99990007'
